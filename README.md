...
> **Status: early development.** The development plugin, offline CLI, result contract, captured-output compaction, existing-check runner and repository lookup are implemented. Remaining runtime operations and skills are planned in [issues #1–#10](https://github.com/fws94/product9-token-saver/issues). There is no published release yet; a development installation with compact-output, run-checks and repo-lookup skills is available.
...
scripts/       Repository checks, CLI, result contract, compaction, checks and lookup helpers
skills/        compact-output, run-checks and repo-lookup skills; other skills remain planned
