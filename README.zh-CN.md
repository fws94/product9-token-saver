...
> **当前状态：早期开发。** 已实现开发版插件、离线 CLI、结果契约、已捕获输出精简、现有检查运行器、精准仓库检索和批量 GitHub 状态查询、已授权 Luna 提交说明及显式工单管理。其余运行时操作与 Skills 仍在 [#1–#10 开发任务](https://github.com/fws94/product9-token-saver/issues)中推进，目前没有正式发布版本；可以安装包含 compact-output、run-checks、repo-lookup、batch-status、luna-submit 和 issue-admin Skills 的开发版插件。
...
skills/        compact-output、run-checks、repo-lookup、batch-status、luna-submit 与 issue-admin Skills；其余 Skills 仍在规划中
tests/         仓库工具的测试
```
